...
public class CompressedRTFInputStream extends InputStream {
	...
	protected static final int MAGIC_COMPRESSED = 0x75465a4c,
	...
	protected void init() throws IOException {
		...
		if (magic == MAGIC_COMPRESSED) {
			...
			throw new IOException("Unknown compression type (magic number "
					+ magic + ")");
		}
	}
		...
		while (true) {
			...
			if (remaining < 3) {
				...
				if (remaining < required) {
					...
					bufstart = 0;
					bufend = TNEFUtils.read(in, buf, remaining, required
							- remaining, buf.length - remaining);
					compressedSize -= bufend;
					bufend += remaining;
					for (int i = remaining; i < bufend; i++)
						// append to CRC
	...
	protected boolean more() throws IOException {
		...
		return magic == MAGIC_COMPRESSED ? moreCompressed()
				: moreUncompressed();
	}
