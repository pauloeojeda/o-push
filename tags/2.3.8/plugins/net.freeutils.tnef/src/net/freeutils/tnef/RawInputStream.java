...
public class RawInputStream extends InputStream {
	...
	void init(InputStream in, long offset, long length) throws IOException {
		if (in instanceof FileInputStream && length > 16)
			in = new BufferedInputStream(in, length < 4096 ? (int) length
					: 4096);
		this.in = in;
		...
		if (skip(offset) != offset)
			throw new IOException("can't skip to offset " + offset
					+ " in stream");
		// init fields
		this.length = length;
		this.offset = offset;
	...
	public long readU64() throws IOException {
		return (readU32() & 0x00000000FFFFFFFFL)
				| ((readU32() & 0x00000000FFFFFFFFL) << 32);
	}
