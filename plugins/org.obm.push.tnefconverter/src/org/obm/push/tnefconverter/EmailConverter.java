...
public class EmailConverter {
	...
	public InputStream convert(InputStream email)
		...
		try {
			...
			if (TNEFExtractorUtils.isScheduleMeetingRequest(message)) {
				...
				if (handler.getCc().size() > 0) {
					basicHeader.setCc(handler.getCc().toArray(new Address[0]));
				}
				...
				textPart.setBody(new CharSequenceSource(meeting.getDescription()));
				root.addChild(textPart);

				MimeHeader requestMimeHeader = new MimeHeader(new Header());
				String method = "REQUEST";
				if(PidTagMessageClass.ScheduleMeetingCanceled.equals(meeting.getMethod())){
					method = "CANCEL";
				}
				requestMimeHeader
						.set("Content-Type",
								"text/calendar; charset=\"UTF-8\"; method="+method+"; charset=\"UTF-8\"");
				requestMimeHeader.setContentTransferEncoding("8bit");
				LocalMimePart requestPart = new LocalMimePart(requestMimeHeader);
				requestPart.setBody(new CharSequenceSource(ics
						.replace("\0", "")
						+ "\n"));
				...
				byte[] b = FileUtils.streamBytes( handler.getTnefDoc(), true);
				tnefPart.setBody(new CharSequenceSource(new String(Base64
						.encode(b))));
				root.addChild(tnefPart);
				InputStream in = MimeTreeRenderer.getInstance().renderMimePart(
						root);
				return in;
